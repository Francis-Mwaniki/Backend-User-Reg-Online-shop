...
else if(emailExist){res.status(400).send({message:"email already in use"})}
else{
   try {
    ...
    res.status(201).send({message:`${user.firstName}, Thank you for creating your account`})
   } catch (error) {
...
router.post('/login', async(req,res)=>{
 ...
 if(!userdt){ return res.status(400).send({message:"Username is not found"});}
 else{
     ...
     if(!validPass){ return res.status(400).send({message:"Email or Password is wrong"});}
        ...
        res.header('auth-token',token).status(200).send({message:`You Logged in as ${userdt.firstName}`});
    }
 }
})
